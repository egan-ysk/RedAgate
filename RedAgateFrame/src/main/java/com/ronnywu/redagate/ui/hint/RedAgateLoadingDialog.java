 ...
 * 加载对话框
 */
public class RedAgateLoadingDialog extends Dialog {
    ...
    public RedAgateLoadingDialog(Context context) {
        super(context, R.style.RedAgate_Dialog);
        init();
    }

    public static void dismissDialog(RedAgateLoadingDialog redAgateLoadingDialog) {
        if (null == redAgateLoadingDialog) {
            return;
        }
        redAgateLoadingDialog.dismiss();
    }

    private void init() {
        setContentView(R.layout.red_agate_loading_dialog);
        iv_route = (ImageView) findViewById(R.id.iv_route);
        detail_tv = (TextView) findViewById(R.id.detail_tv);
        tv_point = (TextView) findViewById(R.id.tv_point);
        initAnim();
        // 添加窗口淡入,淡出动画
        getWindow().setWindowAnimations(R.style.RedAgate_Dialog);
    }
