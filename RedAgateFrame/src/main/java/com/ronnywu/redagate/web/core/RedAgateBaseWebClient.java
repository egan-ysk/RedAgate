package com.ronnywu.redagate.web.core;
...
public class RedAgateBaseWebClient extends WebViewClient {
    ...
    private Map<String, RedAgateJsMessageCallback> responseCallbacks = null;
    private Map<String, RedAgateJsMessageHandler> messageHandlers = null;
    private long uniqueId = 0;
    private RedAgateJsMessageHandler messageHandler;
    private MyJavascriptInterface myInterface = new MyJavascriptInterface();

    public RedAgateBaseWebClient(WebView webView) {
        this(webView, null);
    }

    public RedAgateBaseWebClient(WebView webView, RedAgateJsMessageHandler messageHandler) {
        this.webView = webView;
        this.webView.getSettings().setJavaScriptEnabled(true);
        this.webView.addJavascriptInterface(myInterface, kInterface);
        this.responseCallbacks = new HashMap<String, RedAgateJsMessageCallback>();
        this.messageHandlers = new HashMap<String, RedAgateJsMessageHandler>();
    ...
    public void send(Object data, RedAgateJsMessageCallback responseCallback) {
    ...
    public void callHandler(String handlerName, Object data,
                            RedAgateJsMessageCallback responseCallback) {
        sendData(data, responseCallback, handlerName);
    }

    public void registerHandler(String handlerName, RedAgateJsMessageHandler handler) {
        if (handlerName == null || handlerName.length() == 0 || handler == null)
            return;
        messageHandlers.put(handlerName, handler);
    }

    private void sendData(Object data, RedAgateJsMessageCallback responseCallback,
                          String handlerName) {
    ...
    private void processQueueMessage(String messageQueueString) {
        try {
            ...
            for (int i = 0; i < messages.length(); i++) {
                ...
                if (message.responseId != null) {
                    RedAgateJsMessageCallback responseCallback = responseCallbacks
                            .remove(message.responseId);
                    if (responseCallback != null) {
                        responseCallback.callback(message.responseData);
                    }
                } else {
                    RedAgateJsMessageCallback responseCallback = null;
                    if (message.callbackId != null) {
                        final String callbackId = message.callbackId;
                        responseCallback = new RedAgateJsMessageCallback() {
                            @Override
                    ...
                    RedAgateJsMessageHandler handler;
                    if (message.handlerName != null) {
     ...
     * JS 消息回调.
     */
    public interface RedAgateJsMessageCallback {

        /**
         * 为了保证兼容性,更新UI等操作,请在UI线程操作.
         *
         * @param data JS 传递回来的数据.
         */
        public void callback(Object data);
    }

    public interface RedAgateJsMessageHandler {
        public void request(Object data, RedAgateJsMessageCallback callback);
    }
